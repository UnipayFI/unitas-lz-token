...
export async function sendEvm(
    { srcEid, dstEid, amount, to, minAmount, extraOptions, composeMsg, oftAddress, safeAddress }: EvmArgs,
    ...
    const oft = await srcEidHre.ethers.getContractAt(oftArtifact.abi, wrapperAddress, signer as any)
    // 3️⃣ fetch the underlying ERC-20
    const underlying = await oft.token()
    // 4️⃣ fetch decimals from the underlying token
    const erc20 = await srcEidHre.ethers.getContractAt('ERC20', underlying, signer as any)
    ...
    if (safeAddress) {
        const refundAddress = safeAddress
        const calldata = oft.interface.encodeFunctionData('send', [
            [
                sendParam.dstEid,
                sendParam.to,
                sendParam.amountLD,
