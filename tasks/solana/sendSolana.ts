...
export async function sendSolana({
    ...
    // 1️⃣ RPC + UMI
    const primary = vaultPda ? await deriveConnection(srcEid, { readOnly: true }) : await deriveConnection(srcEid)
    const { connection } = primary
    const { umi, umiWalletSigner } = vaultPda
        ? await deriveConnection(srcEid, { readOnly: true, noopSigner: publicKey(vaultPda) })
        : primary
    ...
    const sendParam = {
        dstEid,
        to: new Uint8Array(addressToBytes32(to)),
        amountLd: amountUnits,
        minAmountLd: minAmount ? parseDecimalToUnits(minAmount, decimals) : amountUnits,
        options: extraOptions ? new Uint8Array(Buffer.from(extraOptions.replace(/^0x/, ''), 'hex')) : undefined,
        composeMsg: composeMsg ? new Uint8Array(Buffer.from(composeMsg.replace(/^0x/, ''), 'hex')) : undefined,
    }
    ...
    txB = await addComputeUnitInstructions(
        ...
        srcEid,
        txB,
        primary.umiWalletSigner as any,
    ...
    try {
        if (vaultPda) {
            txB.setFeePayer(umiWalletSigner).useV0()
            const web3JsTxn = toWeb3JsTransaction(await txB.buildWithLatestBlockhash(umi))

            const fullTxBytes = web3JsTxn.serialize()
            const squadsBase58Message = bs58.encode(new Uint8Array(fullTxBytes))

            console.log('==== Squads Transaction Data ====')
            console.log('Base58 Transaction:')
            console.log(squadsBase58Message)
            console.log('')
