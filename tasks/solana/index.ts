...
import { AddressLookupTableAccount, Connection, MessageV0, VersionedTransaction } from '@solana/web3.js'
...
export const getComputeUnitPriceAndLimit = async (
    ...
    if (skipSimulation) {
        console.log(
            `Skipping simulation, using hardcoded estimate for ${transactionType}: ${TransactionCuEstimates[transactionType]} CUs`
        )
...
/**
 * Serialize a VersionedTransaction's V0 message into the Squads v4
 * VaultTransactionMessage Borsh format.
 *
 * Squads expects: numSigners, numWritableSigners, numWritableNonSigners,
 * accountKeys, instructions, addressTableLookups — all Borsh-encoded
 * (u32-LE lengths for vectors, no version prefix, no recent blockhash).
 */
export function serializeV0MessageForSquads(txn: VersionedTransaction): Uint8Array {
    const msg = txn.message as MessageV0

    const numSigners = msg.header.numRequiredSignatures
    const numWritableSigners = numSigners - msg.header.numReadonlySignedAccounts
    const numWritableNonSigners =
        msg.staticAccountKeys.length - numSigners - msg.header.numReadonlyUnsignedAccounts

    const parts: Buffer[] = []

    // Header: 3 × u8
    parts.push(Buffer.from([numSigners, numWritableSigners, numWritableNonSigners]))

    // accountKeys: Vec<Pubkey>
    const keysLen = Buffer.alloc(4)
    keysLen.writeUInt32LE(msg.staticAccountKeys.length)
    parts.push(keysLen)
    for (const key of msg.staticAccountKeys) {
        parts.push(Buffer.from(key.toBytes()))
    }

    // instructions: Vec<CompiledInstruction>
    const ixLen = Buffer.alloc(4)
    ixLen.writeUInt32LE(msg.compiledInstructions.length)
    parts.push(ixLen)
    for (const ix of msg.compiledInstructions) {
        ...
        accLen.writeUInt32LE(ix.accountKeyIndexes.length)
        parts.push(accLen)
    ...
    const lutLen = Buffer.alloc(4)
    lutLen.writeUInt32LE(lookups.length)
    parts.push(lutLen)
    for (const lut of lookups) {
        // accountKey: Pubkey
        parts.push(Buffer.from(lut.accountKey.toBytes()))
        // writableIndexes: Vec<u8>
        const wLen = Buffer.alloc(4)
        wLen.writeUInt32LE(lut.writableIndexes.length)
        parts.push(wLen)
        parts.push(Buffer.from(lut.writableIndexes))
        // readonlyIndexes: Vec<u8>
        const rLen = Buffer.alloc(4)
        rLen.writeUInt32LE(lut.readonlyIndexes.length)
        parts.push(rLen)
        parts.push(Buffer.from(lut.readonlyIndexes))
    }
