...
task('lz:oft:send', 'Sends OFT tokens cross‐chain from any supported chain')
    ...
    .addOptionalParam(
        'vaultPda',
        'Solana only: token owner vault PDA; outputs a Squads-compatible base58 message instead of sending',
        undefined,
        types.string
    )
    .addOptionalParam(
        'safeAddress',
        'EVM only: Safe multisig address; outputs calldata for Safe transaction proposal instead of sending',
        undefined,
        types.string
    )
    .setAction(async (args: MasterArgs, hre: HardhatRuntimeEnvironment) => {
        ...
        if (result.squadsBase58Message) {
            DebugLogger.printLayerZeroOutput(
                KnownOutputs.EXPLORER_LINK,
                'Built transaction message for Squads. No transaction was sent.'
            )
            console.log('==== Import the following base58 txn data into the Squads UI ====')
            console.log(result.squadsBase58Message)
            return
        }

        if (result.safeTxData) {
            DebugLogger.printLayerZeroOutput(
                KnownOutputs.EXPLORER_LINK,
                'Built transaction data for Safe multisig. No transaction was sent.'
            )
            console.log('==== Safe Transaction Data ====')
            console.log(`To (OFT contract): ${result.safeTxData.to}`)
            console.log(`Value (native fee): ${result.safeTxData.value}`)
            console.log(`Data (calldata):`)
            console.log(result.safeTxData.data)
            console.log('==== Use the above in Safe Transaction Builder ====')
            return
        }

        if (!result.txHash || !result.scanLink) {
            throw new Error('Missing txHash or scanLink in send result')
        }
